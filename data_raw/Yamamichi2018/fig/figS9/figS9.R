...
# Data
read.csv("sestonC.csv",header=TRUE)->dat
da<-t(dat[,-1])


## Fig. S9a
# Time-series of seston carbon
date<-c("2015/6/24","2015/7/10","2015/7/22","2015/8/3","2015/8/16","2015/8/27","2015/9/8","2015/9/21")
type <- unique(dat[, 1])

plot(as.POSIXct(date), log10(dat[1,-1]), type = "n", ylim = c(1.6, 2.7), xlab = "Date in 2015", ylab = expression(paste("log"[10], "(Seston carbon (", mu, "mol ", L^{-1},  "))")), cex.lab=2, xaxt="n")
panelLabel("(a)");
...
legend("topright", legend = dat[, 1], pch = pchs, lty = 1, col = cols)


## Fig. S9b
# Boxplot of seston carbon
boxplot(log10(da[-1,1]), log10(da[-1,2]), log10(da[-1,3]), log10(da[-1,4]), log10(da[-1,5]), log10(da[-1,6]), names=dat[,1], border=cols, xlab="Pond ID", ylab = expression(paste("log"[10], "(Seston carbon (", mu, "mol ", L^{-1},  "))")), cex.lab=2, ylim=c(1.6, 2.7))
panelLabel("(b)");


## Fig. S9c
# Seston carbon vs. macrophyte biomass
...
plot(x, y, xlim=c(0,100), ylim=c(40,310), xlab=expression(paste("Macrophyte biomass (g DW ", m^{-2}, ")")), ylab=expression(paste("Seston carbon (", mu, "mol ", L^{-1},  ")")), pch=19, col=1, cex=2, cex.lab=2)
...
## Fig. S9d
# Seston carbon vs. light intensity
light<-c(24.6, 24.6, 43.5, 43.5, 100, 100)

# 202L
x<-light[1]; y<-plank[1]; y1<-plase[1];
plot(x, y, xlim=c(0,110), ylim=c(40,310), xlab="Percent surface light transmission", ylab=expression(paste("Seston carbon (", mu, "mol ", L^{-1},  ")")), pch=19, col=1, cex=2, cex.lab=2)
arrows(x, y, x, y-y1, angle = 90, length = 0.1, col = 1)
arrows(x, y, x, y+y1, angle = 90, length = 0.1, col = 1)
panelLabel("(d)");
